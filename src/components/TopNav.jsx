...
export default function TopNav({ onMobileMenu }) {
  ...
  // Live notifications
  const liveNotifs  = useQuery(api.notifications.getNotifications, { unreadOnly: false });
  const unreadCount = useQuery(api.notifications.getUnreadCount);
  const markAllReadMutation = useQuery(api.notifications.getUnreadCount); // just for count

  const notifications = liveNotifs?.slice(0, 5).map(n => ({
    title: n.title,
    message: n.message,

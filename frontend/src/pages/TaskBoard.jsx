...
export default function TaskBoard() {
  ...
  const { data: tasks, refetch } = useApi(() => api.getTasks());
  ...
  const handleDelete = async (taskId) => {
    ...
    try { await api.deleteTask(taskId); refetch(); } catch (err) { alert(err.message); }
  };

  const handleStatusChange = async (taskId, newStatus) => {
    try { await api.updateTaskStatus(taskId, newStatus); refetch(); } catch (err) { alert(err.message); }
  };

...
function AddMemberModal({ onClose, onAdded }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.addTeamMember({ name: form.name, email: form.email, role: form.role, department: form.department || undefined });
      onAdded?.();
      onClose();
    } catch (e) {
...
// ─── Member Card ───────────────────────────────────────────────────────────
function MemberCard({ member, onStartChat, onRemove, onRefresh }) {
  const [menuOpen, setMenuOpen] = useState(false);

  const cycleStatus = async () => {
    const next = { offline: 'online', online: 'away', away: 'offline' };
    await api.updateTeamMember(member._id, { status: next[member.status] });
    onRefresh?.();
  };
...
function TeamChat({ members, currentUser }) {
  const { data: messages, refetch: refetchMessages } = useApi(() => api.getTeamMessages(100), { interval: 3000 });
...
export default function TeamHub() {
  const { user } = useAuth();
  const { data: members, refetch } = useApi(() => api.listTeamMembers());
  ...
  const handleRemove = async (id) => {
    ...
    try { await api.removeTeamMember(id); refetch(); } catch (e) { alert(e.message); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'members' && (
        <div className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            <AnimatePresence>
              {filtered.map(m => (
                <MemberCard key={m._id} member={m} onStartChat={handleStartChat} onRemove={handleRemove} onRefresh={refetch} />
              ))}
            </AnimatePresence>
          </div>
        </div>
      ...
      {showAdd && <AddMemberModal onClose={() => setShowAdd(false)} onAdded={refetch} />}
    </div>
  );
}

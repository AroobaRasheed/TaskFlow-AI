...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await api.signup({ name, email, password, role: 'member' });
      if (result?.devToken) setDevCode(result.devToken);
      if (result?.token) setSignupToken(result.token);
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      await api.verifyEmail({ email, token: code });
      login({ name, email, role: 'member', emailVerified: true, token: signupToken });
      setStep('done');
  ...
  const handleResend = async () => {
    ...
    setError('');
    try {
      const result = await api.resendOtp({ email, name });
      if (result?.devToken) setDevCode(result.devToken);
      startResendCooldown();
    } catch (err) {
